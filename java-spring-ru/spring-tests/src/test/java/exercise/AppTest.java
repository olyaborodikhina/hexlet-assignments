...
public class AppTest {
    ...
    @Test
    void testPatchPeopleById() throws Exception {
        MockHttpServletResponse responseUpd = mockMvc
                .perform(
                        // Выполняем POST-запрос
                        patch("/people/1")
                                // Устанавливаем тип содержимого тела запроса
        ...
        assertThat(responseUpd.getStatus()).isEqualTo(200);
        MockHttpServletResponse response = mockMvc
                .perform(get("/people"))
                .andReturn()
                .getResponse();
    ...
    @Test
    void testDeletePeopleById() throws Exception {
        ...
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        // Проверяем, что тело ответа не содержит данные сущностей
        assertThat(response.getContentAsString()).doesNotContain("John", "Smith");
    }
    // END
}
